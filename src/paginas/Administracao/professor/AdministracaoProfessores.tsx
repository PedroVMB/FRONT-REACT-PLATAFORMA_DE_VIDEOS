...
const AdministracaoProfessores = () => {
    const [coordenadores, setCoordenadores] = useState<IUser[]>([]);

    useEffect(() => {
        http.get<ApiResponse>('professor')
            .then(resposta => {
    ...
    const excluir = (coordenadorAhSerExcluido: IUser) => {
        http.delete(`professor/${coordenadorAhSerExcluido.id}`)
            .then(() => {
                    ...
                    <TableBody>
                        {Array.isArray(coordenadores) && coordenadores.map(coordenador => (
                                <TableRow key={coordenador.id}>
                                    ...
                                    <TableCell>
                                        <RouterLink to={`/admin/professor/${coordenador.id}`}>editar</RouterLink>
                                    </TableCell>
                                    <TableCell>
...
export default AdministracaoProfessores;
