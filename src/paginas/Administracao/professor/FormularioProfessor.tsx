import {
  ...
  export const FormularioProfessor = () => {
    ...
    useEffect(() => {
      if (parametros.id) {
        http.get<IUser>(`professor/${parametros.id}`).then((resposta) => {
    ...
    const aoSubmeterForm = (evento: React.FormEvent<HTMLFormElement>) => {
      ...
      if (parametros.id) {
        http.put(`professor`, user).then(() => {
          alert("professor atualizado com sucesso!");
        });
      } else {
        http.post("professor", user).then(() => {
    ...
    return (
      <>
        <Card>
          <CardHeader title="Formulário de Cadastro Professor" />
